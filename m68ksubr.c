...
int m68k_execute_subroutine(long sp, long subroutine)
{
    ...
    m68k_set_reg(M68K_REG_PC, (long) subroutine);
    m68k_set_reg(M68K_REG_SR, 0x0300);
    m68k_set_reg(M68K_REG_SP, (long) sp);
    ...
    if(!CPU_STOPPED)
    {
        ...
        do
        {
             ...
             */
            if (REG_IR == 0x4e75 && * (unsigned long *) REG_SP == RETURN_STACK_MARKER)
            {
